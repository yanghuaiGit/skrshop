...
(
    ...
    `idcard_no`        varchar(20) not null default '' comment '身份证号',
    `fidcard_name`    varchar(255) not null default '' comment '身份证名'
...
(
    ...
    `create_at`      datetime not null default CURRENT_TIMESTAMP '0' COMMENT '创建时间',
...
(
    ...
    `update_at` datetime not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP  COMMENT '更新时间',
    `modify_at` int(11)               NOT NULL DEFAULT '0' COMMENT '修改人staff_id',
    `status`    tinyint(1)            NOT NULL DEFAULT '0' COMMENT '状态 1:enable, 0:disable, -1:deleted',
    PRIMARY KEY (`id`),
